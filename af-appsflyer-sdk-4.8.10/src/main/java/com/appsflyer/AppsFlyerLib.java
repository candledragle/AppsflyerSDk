...
public class AppsFlyerLib implements AFInstallReferrerListener {
    ...
    static final String JENKINS_BUILD_NUMBER = "381";
    static final String BUILD_NUMBER = "4.8.10";
...
//            }
    ...
    public void setPreinstallAttribution(String mediaSource, String campaign, String siteId) {
        ...
        JSONObject preinstallParams = new JSONObject();
        try {
            if (mediaSource != null) {
                preinstallParams.put(Constants.URL_MEDIA_SOURCE, mediaSource);
            }
            if (campaign != null) {
                preinstallParams.put(Constants.URL_CAMPAIGN, campaign);
            }
            if (siteId !=null){
                preinstallParams.put(Constants.URL_SITE_ID, siteId);
            }

        } catch (JSONException jsex ){
            AFLogger.afErrorLog(jsex.getMessage(),jsex);
        }

        if (preinstallParams.has(Constants.URL_MEDIA_SOURCE)) {
            setProperty(PRE_INSTALL_PREF, preinstallParams.toString());
        }
        else {
